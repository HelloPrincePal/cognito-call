...
- **PyTorch Hub Headless Trust Bypass (`transcriber.py`):** Overrode PyTorch Hub's interactive trusted repository warning checker (`torch.hub._check_repo_is_trusted`) to automatically return `True`, preventing terminal stdin blockages and `EOFError` during headless Silero VAD downloads.
...
- **Global App Exit & Window Close Cleanup Hooks (`lib.rs`):** Registered callbacks on Tauri `Exit` and `CloseRequested` events to force-terminate all running `cognito-assistant` helper processes (`pkill -9 -f cognito-assistant`), guaranteeing no zombie helper scripts survive application closure.
