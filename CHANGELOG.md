...
- **Diarizer Writeable Model Cache (`transcriber.py`):** Passes an explicit `savedir=~/.cognitocall/pretrained_models` argument to the `Diarizer()` constructor, fully redirecting SpeechBrain model downloads away from the read-only default `pretrained_models/` relative path.
