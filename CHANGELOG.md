...
- **Global App Exit & Window Close Cleanup Hooks (`lib.rs`):** Registered callbacks on Tauri `Exit` and `CloseRequested { api: _ }` window events to force-terminate all running `cognito-assistant` helper processes (`pkill -9 -f cognito-assistant`), guaranteeing no zombie helper scripts survive application closure. Resolves Rust compilation pattern destructuring mismatch (E0533).
